...
util.open = function(url) {
  ...
  a.setAttribute('id', 'ldyxb-link-temp')
  document.body.appendChild(a)
  a.click()
  document.body.removeChild(document.getElementById('ldyxb-link-temp'))
}
 ...
 * @returns {*}
 */
util.HmacSHA1 = function(str) {
  var CryptoJS = require('crypto-js')
  return CryptoJS.HmacSHA1(str, process.env.VUE_APP_SECRET).toString()
}
/**
 * 将任意对象转化为树
 * @param data
...
util.getSign = function(params) {
  ...
  return util.HmacSHA1(basestring)
}
...
util.getImageCodeUrl = function(url, code = '') {
  ...
  data += '/v3/storage/method/get.storage.thumb/code/' + code
...
util.getDownloadUrl = function(file, code) {
  let data = process.env.VUE_APP_BASE_API
  data += '/v3/storage/method/get.storage.download/code/' + code
...
util.getImageStyleUrl = function(url, style) {
  let data = process.env.VUE_APP_BASE_API
  data += '/v3/storage/method/get.storage.thumb' + '?url=' + encodeURIComponent(url)
  data += style
