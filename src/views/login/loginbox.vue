      ...
      <el-row :span="34">
        <el-col :span="14">
          <el-input size="small" @keyup.enter.native="handleLogin()" :maxlength="code.len" v-model="loginForm.code" auto-complete="off" placeholder="请输入验证码">
            <i slot="prefix" class="fa fa-shield"></i>
          </el-input>
        </el-col>
        <el-col :span="10">
          <div class="login-code">
            <!-- <span class="login-code-img" @click="refreshCode" v-if="code.type === 'text'">{{code.value}}</span> -->
            <img :src="code.src" class="login-code-img" @click="refreshCode" alt=""/>
          </div>
        </el-col>
      </el-row>
    </el-form-item>
...
export default {
  ...
  methods: {
     ...
     */
    refreshCode() {
      this.code.src = '/interface/v3/admin?method=get.captcha&platform=admin&id=login&t=' + new Date().getTime()
