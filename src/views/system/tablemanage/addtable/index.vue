      ...
      >
        ...
        <el-form-item label="数据库表名" prop="table_name">
          <el-select
            ...
            collapse-tags:placeholder="请选择数据库表名"
            @visible-change="change(formInline)"
          >
            <div class="el-input" style="width:90%;margin-left:5%;">
              <input
                ...
                v-model.trim="formInline.dropDownValue"
                @input="dropDownSearch(formInline)"
              />
            </div>
            <el-option
...
import { getTableFields, addScene } from '@/api/tablemanage/table'
...
export default {
  ...
  filters: {},
  mounted() {
    requestUrl('/admins/column/Table/sqlTable').then(response => {
  ...
  methods: {
    dropDownSearch(item) {
      ...
      this.optionsMetaShow = this.selects
      this.optionsMetaShow = this.selects.filter(function(v) {
    ...
    change(item) {
      ...
      if (this.formInline.table_name) {
        getTableFields({
          ...
          this.formInline.column_list = res.data
        })
      }
    },
    // 设置配置数据
