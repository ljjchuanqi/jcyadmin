<template>
  <div class="yxb-p">
    ...
    <el-breadcrumb class="breadcrumb yxb-mb" separator-class="el-icon-arrow-right">
      <el-breadcrumb-item>
        ...
        <li v-for="(item, index) in currentTableData" :key="index">
          <dl>
            <dt>
              ...
              <span class="storage-name yxb-ml-10" :title="item.name">{{item.name}}</span>
              <el-dropdown
                placement="bottom"
                :show-timeout="50"
                size="small"
                @command="command => handleControlItemClick(command)">
                <yxb-icon class="more" name="angle-double-down"/>
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item
                    v-if="auth.rename"
                    :command="{type: 'rename', index}">
                    <yxb-icon name="edit" class="more-icon"/>重命名
                  </el-dropdown-item>
                  ...
                  <el-dropdown-item
                    ...
                    divided>
                    <yxb-icon name="cloud-upload" class="more-icon"/>替换上传
                  </el-dropdown-item>

                  <el-dropdown-item
                    v-if="item.type === 0 && item.parent_id && auth.cover"
                    :command="{type: 'cover', index}">
                    <yxb-icon name="image" class="more-icon"/>设为封面
                  </el-dropdown-item>

                  <el-dropdown-item
                    v-if="item.type === 3 && !item.cover && auth.cover"
                    :command="{type: 'video_cover', index}">
                    <yxb-icon name="image" class="more-icon"/>选择海报
                  </el-dropdown-item>

                  <el-dropdown-item
                    v-if="item.type === 2 && auth.default"
                    :command="{type: 'default', index}">
                    <yxb-icon name="folder-open-o" class="more-icon"/>{{item.is_default ? '取消默认' : '设为默认'}}
                  </el-dropdown-item>
                  ...
                  <el-dropdown-item
                    ...
                    divided>
                    <yxb-icon name="image" class="more-icon"/>{{item.type === 2 ? '取消封面' : '取消海报'}}
                  </el-dropdown-item>
                  ...
                  <el-dropdown-item
                    ...
                    divided>
                    <yxb-icon name="arrows" class="more-icon"/>转移目录
                  </el-dropdown-item>

                  <el-dropdown-item
                    v-if="auth.del"
                    :command="{type: 'delete', storage_id: item.storage_id}">
                    <yxb-icon name="trash-o" class="more-icon"/>删除资源
                  </el-dropdown-item>

                  <el-dropdown-item
                    v-if="item.type === 0 && auth.refresh"
                    :command="{type: 'refresh', index}">
                    <yxb-icon name="refresh" class="more-icon"/>清除缓存
                  </el-dropdown-item>
                  ...
                  <el-dropdown-item
                    ...
                    divided>
                    <yxb-icon name="external-link" class="more-icon"/>复制外链
                  </el-dropdown-item>
                </el-dropdown-menu>
              </el-dropdown>
            </dt>
            <dd class="yxb-ml-10">
              ...
              <p>
                ...
                <span v-else>类型：<yxb-icon :name="item.type | getFileTypeIocn"/></span>
              </p>
            </dd>
          </dl>
        </li>
...
export default {
  ...
  components: {
    ...
    'yxbStorage': () => import('@/components/yxb-storage')
  },
  props: {
    tableData: {
      default: () => []
