...
import { addScene, editScene, delScene } from '@/api/common/index'
...
export default {
  ...
  data() {
    return {
      ...
      currentHeaderData: [],
      currentOperateData: {},
      addBodyData: [{ label: '操作', name: '导出', type: -3, is_list: 1, filter: 'state', filterval: 2 }],
      multipleSelection: [],
      auth: {
        add: true,
        set: true,
  ...
  methods: {
    ...
    handleGoDetail(index) {
      console.log(index)
      let that = this
      window.open(that.tableData[index].file_path)
    },
    // 批量删除
    handleDelete(val) {
