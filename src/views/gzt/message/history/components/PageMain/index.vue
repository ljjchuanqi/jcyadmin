  ...
  <div class="yxb-p">
    <el-form :inline="true" size="small">
      <el-form-item v-if="auth.add && currentOperateData.can_add == 1">
        <el-button :disabled="loading" @click="handleCreate">
          <yxb-icon name="plus" />
          新增
        </el-button>
      </el-form-item>
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogDetailVisible = false" size="small"
          >关闭</el-button
        >
        <el-button
          type="primary"
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      ...
      this.$confirm('确定要导出数据吗?', '提示', {
        ...
        .then(() => {
          Promise.all([
            addExport({
              acid: that.activeAcid,
              mform_id: that.$route.query.id,
              mform_title: this.mform_title
            })
          ])
            .then(res => {
    ...
    create() {
      this.$refs.pageedit.startValidate(valid => {
        if (valid) {
          ...
          Promise.all([addScene({ ...this.form })])
            .then(res => {
    ...
    update() {
      this.$refs.pageedit.startValidate(valid => {
        if (valid) {
          ...
          Promise.all([editScene({ ...this.form })])
            .then(res => {
              if (res[0] && res[0].code === -1) {
                this.$message.error('暂无权限')
                this.dialogLoading = false
    ...
    handleGoDetail(index) {
      ...
      this.formDetail = {}
      getHistoryList(this.currentTableData[index].id)
        .then(res => {
