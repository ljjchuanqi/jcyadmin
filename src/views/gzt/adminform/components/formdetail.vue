  ...
  <div>
    <el-form :model="form" label-width="110px" :rules="rules" ref="form">
      ...
      <el-form-item
        name="reg"
        label="输入类型:"
        v-if="form.type.toString() === '1' && !form.foreign_key"
      >
      ...
      <el-form-item name="remark" label="预约设置:" v-if="form.type==15">
        <el-button @click="handleToggleForwardForm()"
          ><yxb-icon class="" name="edit" />编辑</el-button
        >
      </el-form-item>
      ...
      <el-form-item
        name="validate_rule"
        label="验证规则:"
        v-if="!['6', '7', '8', '9', '13'].includes(form.type.toString())"
      >
        <el-input
        ...
        <el-popover placement="right" width="400" trigger="click">
          ...
          <div class="tssm-wrap">
            ...
            <el-row :gutter="20">
              <el-col :span="12"
                >alphaDash(字段的值为字母和数字，下划线_及破折号-)</el-col
              >
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12"
                >chsAlphaNum(字段的值只能是汉字、字母和数字)</el-col
              >
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12"
                >chsDash(字段的值只能是汉字、字母、数字和下划线_及破折号-)</el-col
              >
              <el-col :span="12"
                >cntrl(字段的值只能是控制字符（换行、缩进、空格）)</el-col
              >
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12"
                >graph(字段的值只能是可打印字符（空格除外）)</el-col
              >
              <el-col :span="12"
                >print(字段的值只能是可打印字符（包括空格）)</el-col
              >
            </el-row>
            <el-row :gutter="20">
            ...
            <el-row :gutter="20">
              <el-col :span="12"
                >space(字段的值只能是空白字符（包括缩进，垂直制表符，换行符，回车和换页字符）)</el-col
              >
            ...
            <el-row :gutter="20">
              <el-col :span="12"
                >ip(字段的值为有效的IP地址支持验证ipv4和ipv6格式的IP地址)</el-col
              >
              <el-col :span="12"
                >dateFormat:format(字段的值为指定格式的日期，例如：'create_time'=>'dateFormat:y-m-d')</el-col
              >
            </el-row>
            <el-row :gutter="20">
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="12"
                >zip(字段的值为有效的邮政编码长度和区间验证类)</el-col
              >
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12">in(字段的值在某个范围，例如：in:1,2,3)</el-col>
              <el-col :span="12"
                >notIn(字段的值不在某个范围，例如：notIn:1,2,3)</el-col
              >
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12"
                >between(字段的值在某个区间，例如：between:1,10)</el-col
              >
              <el-col :span="12"
                >notBetween(字段的值不在某个范围，例如：notBetween:1,10)</el-col
              >
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12"
                >length:num1,num2(字段的值的长度在某个范围，例如：length:4,25)</el-col
              >
              <el-col :span="12">length:num(指定长度 例如length:4)</el-col>
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12"
                >max:number(字段的值的最大长度，例如：max:25)</el-col
              >
              <el-col :span="12"
                >min:number(字段的值的最小长度，例如：min:5)</el-col
              >
            </el-row>
            <el-row :gutter="20">
              <el-col :span="12"
                >after:日期(字段的值在某个日期之后，例如：after:2016-3-18)</el-col
              >
              <el-col :span="12"
                >before:日期(字段的值在某个日期之前，例如：before:2016-10-01)</el-col
              >
            </el-row>
          </div>
          <el-button
      ...
      <el-form-item
        name="is_show_front"
        label="页面展示:"
        v-if="!form.foreign_key"
      >
        <el-switch
          v-model="form.is_show_front"
          :active-value="1"
          :inactive-value="0"
        >
        </el-switch>
      ...
      <el-form-item name="is_modify" label="提交通过后:">
        ...
        <span class="desc-info"
          >说明：设置表单审核后再次提交表单时，且该字段为审核通过时，该字段是否可以修改。</span
        >
      </el-form-item>
      <!-- <el-form-ite
      ...
      <el-form-item
        label="新增选项:"
        v-if="
          form.type.toString() === '7' ||
            form.type.toString() === '8' ||
            form.type.toString() === '9' ||
            form.type.toString() === '13'
        "
      >
        <el-row :gutter="20">
...
export default {
  ...
  filters: {},
  computed: {
    ...mapState('ldyxb/gzt', ['isForwardFormVisble'])
  },
  methods: {
    // 打开预约设置对话框
    handleToggleForwardForm() {
      let val = !this.isForwardFormVisble
      this.$store.commit('ldyxb/gzt/toggleForwardForm', val)
    },
    addSelectItem() {
      if (!this.selectItem) {
