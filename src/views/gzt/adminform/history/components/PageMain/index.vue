...
import addAdminRecord from './add-admin-record/index'
import { saveAdminMformData, editScene, delHistory, addExport, getHistoryDetail, formAudit, formSelectsAudit } from '@/api/gzt/adminform'
    ...
    create() {
      this.$refs.pageedit.startValidate(valid => {
        if (valid) {
          ...
          this.form.form_id = this.$route.query.id// 表单id
          Promise.all([
            saveAdminMformData({ ...this.form })
          ])
            .then(res => {
              debugger
              let that = this
