...
<el-table
      ...
      <template v-for="(item,index) in renderData" >
        ...
        <template slot-scope="scope">
            <span
            v-if="Object.prototype.toString.call(item.option)==='[object Object]'"
            :class="{
              'txtgreen':item.name==='audit_state'&&scope.row[item.name]===2,
              'txtorange':item.name==='audit_state'&&scope.row[item.name]===3}"
            >
              {{item.option[scope.row[item.name]]}}
            </span>
            <span v-else-if="Object.prototype.toString.call(item.option)==='[object String]'">
        ...
        v-else-if="item.is_list==1 && item.type == 3">
         <template slot-scope="scope">
          ...
          <div v-if="scope.row[item.name]">
            （列表只显示一张图片）<br>
          <a target="_blank" :href="scope.row[item.name].split(',')[0]" class="img-wrap">
       ...
       <template v-for="(item2,index2) in addData" >
          ...
          <el-table-column
           ...
           <el-table-column
              ...
              v-if="item2.is_list==1 && item2.type == -3">
               <template slot-scope="scope">
                <!-- 等于某个值才显示 -->
                  <el-button v-if="scope.row[item2.filter] === item2.filterval"
                        @click="goDetail(scope.$index)"
                        size="small"
                        type="text">
                        {{item2.name}}
          ...
          <el-table-column
              ...
              <!-- 开关操作 -->
               <template slot-scope="scope">
                  <el-switch v-model="scope.row[item2.name]"
